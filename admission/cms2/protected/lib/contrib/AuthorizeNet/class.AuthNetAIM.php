...
class AuthnetAIM
{
	...
	//2010-11-03 - bjc
	private $x_line_item = array();

	public function addLineItem($itemID, $itemName, $itemDescription, $itemQuantity = 1, $itemPrice, $itemIsTaxable = "Y") {
		...
		$this->x_line_item[] = "{$itemID}<|>{$itemName}<|>{$itemDescription}<|>{$itemQuantity}<|>{$itemPrice}<|>{$itemIsTaxable}";
	}
    ...
    final public function process($retries = 3)
    {
		...
		if (count($this->x_line_item) > 0) {
			foreach ($this->x_line_item as $idx => $line) {
				$query .= "&x_line_item=" . urlencode($line);
			}
		}
		//vd($query);die();
		$count = 0;
