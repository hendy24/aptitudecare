...
<br />
<h2 class="text-center">Admissions for Zip Code {$zip_code}</h2>
...
<table id="report-table" cellpadding="5" cellspacing="0">
	<tr>
		...
		<th><a href="{$SITE_URL}/?page=report&action=details&facility={$facility->pubid}&type=admission&start_date={$start_date}&end_date={$end_date}&filterby={$filterby}&viewby={$viewby}&orderby=admit_date">Address</a></th>
	...
	<tr class="text-left" bgcolor="{cycle values="#d0e2f0,#ffffff"}">
		...
		<td>{$a->address}<br />{$a->city}, {$a->state}</td>
