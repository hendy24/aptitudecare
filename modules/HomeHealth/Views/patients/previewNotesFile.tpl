...
<div id="preview-chunks" style="display: none;">
	...
	<a href="{if $i == $offset}#{else}{$SITE_URL}/?module=HomeHealth&amp;page=Patients&amp;action=previewNotesFile&amp;patient={$patient->public_id}&amp;b={urlencode($b)}&amp;offset={$i}{/if}" class="{if $i == $offset}button-disabled{else}button{/if}" style="margin-right: 5px;">Pages {$i+1} to {if $i + $numPages < $totalPages}{$i+$numPages}{else}{$totalPages}{/if}</a>	
	{/for}
