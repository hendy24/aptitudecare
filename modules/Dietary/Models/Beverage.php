...
class Beverage extends Dietary {
  ...
  public function fetchBeverageReport($location, $date){
    ...
    $sql = "SELECT count(bev.id) AS num, bev.name, pb.meal FROM dietary_beverage bev INNER JOIN dietary_patient_beverage pb ON pb.beverage_id = bev.id INNER JOIN admit_schedule sch ON sch.patient_id = pb.patient_id WHERE sch.location_id = :location_id AND sch.datetime_admit <= :date AND (sch.datetime_discharge >= :date OR sch.datetime_discharge IS NULL) AND sch.status = 'Approved' GROUP BY pb.meal, bev.id ORDER BY pb.meal";
