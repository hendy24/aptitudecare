...
class Snack extends Dietary {
    ...
    snack.name,
    (SELECT GROUP_CONCAT(allergy.name, ' ') FROM dietary_patient_food_info AS patient_allergy INNER JOIN dietary_allergy allergy ON allergy.id=patient_allergy.food_id WHERE patient_allergy.patient_id=patient.id) AS allergy, 
    patient_snack.time 
