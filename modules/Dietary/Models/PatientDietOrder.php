...
class PatientDietOrder extends Dietary {
  ...
  public function fetchPatientCensus($location_id, $order_by = false) {
    ...
    $sql = "SELECT 
              ...
              (SELECT GROUP_CONCAT(t.name separator ', ') as texture FROM {$texture->tableName()} t INNER JOIN {$patient_texture->tableName()} pt ON pt.texture_id = t.id WHERE t.is_liquid = 0 AND pt.patient_id = p.id GROUP BY pt.patient_id) AS texture,
              (SELECT GROUP_CONCAT(t.name separator ', ') as texture FROM {$texture->tableName()} t INNER JOIN {$patient_texture->tableName()} pt ON pt.texture_id = t.id WHERE is_liquid = 1 AND pt.patient_id = p.id GROUP BY pt.patient_id) AS liquid_order,
