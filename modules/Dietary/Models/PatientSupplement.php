...
class patient_supplement extends Dietary {

  protected $table = "patient_supplement";

  public function fetchPatientSupplement($patient_id) {
    $supplement = $this->loadTable("Supplement");
    $sql = "SELECT * FROM {$this->tableName()} po inner JOIN {$supplement->tableName()} AS d ON d.id = po.supplement_id and po.patient_id = :patient_id";
