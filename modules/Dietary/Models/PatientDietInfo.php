...
class PatientDietInfo extends Dietary {
  ...
  public function fetchDietInfoByPatient($patient_id) {
    $diet_info = $this->loadTable("DietInfo");
    $sql = "SELECT GROUP_CONCAT(d.name separator ', ') AS list FROM {$this->tableName()} pdi right JOIN {$diet_info->tableName()} AS d ON d.id = pdi.diet_info_id WHERE pdi.patient_id = :patient_id";
    $params[":patient_id"] = $patient_id;
