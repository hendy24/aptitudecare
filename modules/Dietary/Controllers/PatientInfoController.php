...
/*
 * -------------------------------------------------------------------------
 *  TRANSFER RESIDENT TO NEW ROOM
 * -------------------------------------------------------------------------
 */
	public function move_rooms() {
		// get resident room info
		$patient = $this->loadModel('Client', input()->patient);
		$schedule = $this->loadModel('Schedule', $patient->id);
		$room = $this->loadModel('Room', $schedule->room);
		$location = $this->loadModel('Location', input()->location);

		// get available rooms
		$available_rooms = $this->loadModel('Room')->fetchEmpty($location->id);
		
		smarty()->assign('resident', $patient);
		smarty()->assign('availableRooms', $available_rooms);
